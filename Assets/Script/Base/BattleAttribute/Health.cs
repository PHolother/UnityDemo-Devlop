...
namespace Script.Base.BattleAttribute
{
    ...
    public class Health : MonoBehaviour, IDamageable
    {
        ...
        {
            HealthProbe("TakeDamage 进入 damage=" + damage + " cur=" + currentHealth + " isDead=" + isDead);
            // 如果已经死亡，不再受到伤害
            if (isDead)
            {
                return false;
            ...
            // 防御判定：扣血前询问父链上所有 IDamageDefender，
            // 任一声明防住则本次伤害完全免伤（不扣血、不触发 onHealthChanged）
            var defenders = GetComponentsInParent<IDamageDefender>(false);
            foreach (var defender in defenders)
            {
                if (defender != null && defender.TryDefend(damage, null))
                {
                    return false;
                }
            }
            ...
            HealthProbe("TakeDamage 扣血完成 prev=" + previousHealth + " cur=" + currentHealth + " change=" + healthChange);
        ...
        /// <summary>
        /// 恢复血量并复活（showcase 展示/测试用）：设满血、清除死亡状态、触发事件。
        /// </summary>
        public void RestoreHealth(int amount = -1)
        {
            ...
            isDead = false;
            onHealthChanged.Invoke(0); // 通知 UI 刷新（变化量 0，UI 读当前值）
            HealthProbe("RestoreHealth 恢复至 cur=" + currentHealth + " max=" + maxHealth + " isDead=" + isDead);
        }
        ...
        /// </summary>
        private void HealthProbe(string msg)
        {
            try
            {
                ...
                System.IO.File.AppendAllText(path, "t=" + Time.time.ToString("0.000") + " [Health]" + msg + System.Environment.NewLine);
            }
