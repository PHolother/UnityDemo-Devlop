...
namespace Script.Base.BattleAttribute
{
    ...
    public class BattleAttributes : MonoBehaviour, IDamageProvider
    {
        ...
        [Tooltip("当前攻击段的伤害（由 PlayerAttack 按连段设置，-1=用 baseDamage）")]
        private float currentAttackDamage = -1f;
        ...
        {
            if (currentAttackDamage > 0f)
            {
                // 连段攻击伤害（float，取整）+ 加成
                return Mathf.RoundToInt(currentAttackDamage + damageBonus);
            }
            return baseDamage + damageBonus;
        }

        /// <summary>
        /// 设置当前攻击段伤害（PlayerAttack 按连段调用，-1 恢复用 baseDamage）。
        /// </summary>
        public void SetCurrentAttackDamage(float damage)
        {
